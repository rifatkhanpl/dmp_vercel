...
export function TemplateUpload() {
  ...
  const { loading: isProcessing, execute: processFileAsync } = useAsyncOperation(
    async () => {
      if (!uploadedFile) throw new Error('No file selected');
      return await DMPService.processTemplateUpload(uploadedFile);
    },
    {
      onSuccess: (job) => {
        setImportJob(job);
        errorService.showSuccess('File processed successfully');
      },
      onError: (error) => {
        errorService.showError(`Processing failed: ${error}`);
      }
    }
  );
  ...
  const handleFile = async (file: File) => {
    ...
    const validation = await ValidationService.validateFileUpload(file);
    ...
    try {
      ...
      errorService.logError(error as Error, { context: 'File preview generation', fileName: file.name });
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <button
                  ...
                  aria-label="Download CSV template for data import"
                >
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            {!uploadedFile ? (
              <div
                ...
                role="button"
                tabIndex={0}
                aria-label="File upload area - drag and drop or click to select files"
                onKeyDown={(e) => {
                ...
                {!importJob && (
                  <div className="flex space-x-3">
                    <button
                      ...
                      aria-label="Process and validate uploaded file"
                    >
                      {isProcessing ? (
                        <>
                    ...
                    <button
                      ...
                      aria-label="Preview file contents"
                    >
